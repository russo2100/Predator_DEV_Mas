...
class GWDDEngine:
    ...
    def decide_entry(
        ...
        if len(self.weight_history) > 0:
            ma5 = float(np.mean(self.weight_history))
        ...
        if momentum < -0.05:
            return False, float(entry_weight), (
                f"SKIP: GWDD momentum collapse, "
                f"momentum={momentum:.3f} < -0.05 (weight={entry_weight:.2f}, MA5={ma5:.2f})"
            )
        ...
        # Пропускаем entry если:
        # - weight > 0.55 ИЛИ
        # - MA5 > 0.52 И momentum > -0.02 (восходящий тренд)
        gwdd_v2_pass = (
            entry_weight > 0.55 or
            (ma5 > 0.52 and momentum > -0.02)
        )
